		...
		<div class="leftblock__nav noselect">
			{% if is_verify_user %}
				<ul>
					...
					{% if not access.nav or access.nav.guides %}<li><a href="account/guides"><svg><use xlink:href="#book"></use></svg><span>Образование</span></a></li>{% endif %}
					...
					{% if not access.nav or access.nav.timesheet %}<li timesheetperiodbutton><svg><use xlink:href="#calendar"></use></svg><span>Расписание</span></li>{% endif %}
					{% if not access.nav or access.nav.wallet %}<li reportpatternsbutton><svg><use xlink:href="#wallet"></use></svg><span>Выплаты</span></li>{% endif %}
					{% if not access.nav or access.nav.information %}<li importantinfo><svg><use xlink:href="#information"></use></svg><span>Важная информация</span></li>{% endif %}
					{% if not access.nav or access.nav.operators %}<li operators><svg><use xlink:href="#information"></use></svg><span>Операторы</span></li>{% endif %}
					{% if role and statics|length > 0 %}
						{% if not access.nav or access.nav.offtime %}<li getofftime><svg><use xlink:href="#weekend"></use></svg><span>Выходные</span></li>{% endif %}
						{% if not access.nav or access.nav.vacation %}<li getvacation><svg><use xlink:href="#vacation"></use></svg><span>Отпуск</span></li>{% endif %}
					{% endif %}
					{% if not access.nav or access.nav.personages %}<li getpersonages><svg><use xlink:href="#weekend"></use></svg><span>Мои персонажи</span></li>{% endif %}
					{% if not access.nav or access.nav.paymentorders %}<li paymentorders><svg><use xlink:href="#weekend"></use></svg><span>Мои заявки</span></li>{% endif %}
				</ul>	
			{% endif %}
		</div>
	</div>
		...
		<div class="leftblock__nav leftblock__nav_bottom noselect">
			{% if is_verify_user %}
				<ul>
					...
					<li onclick="window.open('{{callbackform_setting}}')"><svg><use xlink:href="#message"></use></svg><span>Форма для обращений</span></li>
