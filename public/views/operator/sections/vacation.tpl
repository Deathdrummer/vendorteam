<div class="section" id="vacationSection">
	...
	<div class="section__buttons" id="sectionButtons">
		<button id="vacationSave" title="Сохранить настройки"><i class="fa fa-save"></i></button>
	</div>
...
$(document).ready(function() {
	...
	var weeksShift = 0;
	if ($('#vacationSection').find('#tabVacationData').hasClass('active')) {
		getVacationDataToOperator();
	}
	...
	function getVacationDataToOperator(callback) {
		getAjaxHtml('vacation/get_data_to_admin', {weeks_shift: weeksShift}, function(html) {
			...
			if (hashData[2] != undefined) {
				if ($('#vacationSection').find('.tabstitles:not(.sub)').siblings('.tabscontent').find('.tabstitles.sub').length > 0) {
					$('#vacationSection').find('.tabstitles:not(.sub)').siblings('.tabscontent').find('.tabstitles.sub').each(function() {
			...
			$('#vacationSection').multiChoose({
				item: '[setvacation]',
				cls: 'choosed'
			}, function(cellsRow, choosedItems, isChoosed) {
				if ($('#vacationSection').find('[setvacation].choosed').length > 0) {
			...
			$('[shiftweeks]').on(tapEvent, function() {
				...
				getVacationDataToOperator(function() {
					//$('#vacationTable').removeClass('loading');
				});
			});
			...
			$('[vacationaction]').on(tapEvent, function() {
				...
				if (action == 'set') items = $('#vacationSection').find('[setvacation].choosed:not(.vacated)');
				else if (action == 'confirm') items = $('#vacationSection').find('[setvacation].choosed');
				else if (action == 'remove') items = $('#vacationSection').find('[setvacation].choosed.vacated');
				...
				if (choosedDays.length == 0) {
					$('#vacationSection').find('[setvacation].choosed').removeClass('choosed');
 					$(vacationActionButtons).find('button').setAttrib('disabled');
					return false;
				} 
	...
	$('#vacationTableGlobalDisabled').multiChoose({
		...
		if ($('#vacationSection').find('[vacationglobaldisabledday].choosed').length > 0) {
