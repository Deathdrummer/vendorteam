<div class="section" id="timesheetSection">
	...
	<div class="section__buttons" id="sectionButtons">
		...
		<button id="timesheetPeriodButton" title="Периоды расписания"><i class="fa fa-list-alt"></i></button>
		<p id="timesheetPeriodName"></p>
	</div>
...
$(document).ready(function() {
	...
	$('body').off(tapEvent, '#timesheetPeriodButton').on(tapEvent, '#timesheetPeriodButton', function() {
		popUp({
	...
	$('body').off(tapEvent, '[choosetimesheetperiod]').on(tapEvent, '[choosetimesheetperiod]', function() {
		...
		$.post('/timesheet/get_timesheet_data', {period_id: thisId, to_operator: 1}, function(html) {
			$('#timesheetTable').html(html);
			$('#timesheetPeriodName').html('<small>Период:</small> <strong>'+periodName+'</strong>');
		}).fail(function(e) {
			notify('Системная ошибка!', 'error');
			showError(e);
		});
					...
					if (stat) {
						...
						$.ajax({
							...
							success: function(response) {
								if (response) {
									$.post('/timesheet/get_timesheet_data', {period_id: thisPeriodId, to_operator: 1}, function(html) {
										$('#timesheetTable').html(html);
	...
	$('body').off(tapEvent, '[removetimesheetitem]').on(tapEvent, '[removetimesheetitem]', function() {
		var thisItem = this,
			thisPeriodId = $('#timesheetPeriodId').val(),
			thisId = $(thisItem).attr('removetimesheetitem');
		
		popUp({
			title: 'Удалить рейд',
		    width: 400,
		    buttons: [{id: 'deleteTimesheetRaid', title: 'Удалить'}],
		    closeButton: 'Отмена',
		}, function(deleteRaidWin) {
			$('#deleteTimesheetRaid').on(tapEvent, function() {
				deleteRaidWin.wait();
				$.post('/timesheet/remove_timesheet_raid', {id: thisId}, function(response) {
					if (response) {
						$(thisItem).closest('.timesheet__raid').remove();
