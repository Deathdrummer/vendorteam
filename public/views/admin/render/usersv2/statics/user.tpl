{% if statics %}
	<ul class="usersv2staticslist">
		{% for stId, static in statics %}
			<li>
				<input type="checkbox" id="userstatic{{stId}}" usersv2staticlistitem="{{stId}}"{% if static.checked %} checked{% endif %}>
				<label for="userstatic{{stId}}" class="usersv2staticsitem">
					...
					<span class="usersv2staticsitem__checks">
						<div>
							<input type="checkbox" id="userstaticLider{{stId}}" usersv2staticlider="{{stId}}"{% if static.lider %} checked{% endif %}>
							<label for="userstaticLider{{stId}}"></label>
						</div>
						<div>
							<input type="radio" name="userstaticMain" id="userstaticMain{{stId}}" usersv2staticmain="{{stId}}"{% if static.main %} checked{% endif %}>
